...
# else:
#     st.error("No training data found! Please check the 'photo' folder and file names.")
#     st.stop()
# # CSV filename
# current_date = datetime.now().strftime("%Y-%m-%d")
# csv_filename = f"{current_date}.csv"
...
st.set_page_config(page_title="Face Detection & Attendance System", layout="wide")
st.title("📸 Face Detection & Attendance System (Live)")
...
for idx, name in enumerate(known_faces_names):
    ...
    if os.path.exists(img_path):
        img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
        img = cv2.resize(img, (200, 200))  # resize training image
        training_data.append(img)
        labels.append(idx)
...
if training_data and labels:
    ...
    st.error("No training data found in 'photo' folder.")
    st.stop()
...
if st.button("🗑️ Clear Attendance Data"):
    with open(csv_filename, "w") as f:
        f.write("Name,Time\n")
    students = known_faces_names.copy()
    st.success("✅ Attendance data cleared. Ready for new scans!")
# -----------------------------------------------------------------------
...
class FaceDetector(VideoTransformerBase):
    def transform(self, frame):
        ...
        for (x, y, w, h) in faces:
            face_gray = cv2.resize(gray[y:y+h, x:x+w], (200, 200))  # resize detected face
            face_id, confidence = recognizer.predict(face_gray)

            if confidence < 100:  # improved threshold
                name = known_faces_names[face_id]
                color = (0, 255, 0)
                if name in students:
                    students.remove(name)
