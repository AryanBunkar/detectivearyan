...
st.set_page_config(page_title="Face Detection & Attendance System", layout="wide")
st.title("📸 Face Detection & Attendance System (Streamlit Camera)")
...
# CSV filename for today's attendance
current_date = datetime.now().strftime("%Y-%m-%d")
csv_filename = f"{current_date}.csv"
if not os.path.exists(csv_filename):
    with open(csv_filename, "w") as f:
...
# Camera snapshot
st.info("📷 Click 'Take Photo' to capture an image and mark attendance.")
